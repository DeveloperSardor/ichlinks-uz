...
const LocalList = () => {
  ...
  const handleAddDoc = async () => {
    try {
      const response = await axios.post(`${BACKEND_URL}/api/local-list`, newDoc);
  ...
  const handleDeleteDoc = async (id) => {
    try {
      await axios.delete(`${BACKEND_URL}/api/local-list/${id}`);
  ...
  const handleUpdateDoc = async () => {
    if (!editDoc || !editDoc._id) {
      console.error('No document selected or local list ID is missing');
      return;
    }

    try {
      const response = await axios.put(`${BACKEND_URL}/api/local-list/${editDoc._id}`, {
    ...
    <div className="page-container">
      ...
      <div className="docs-content">
        <div className="docs-header">
          <h1>{t('localList')}</h1> {/* Translated Docs title */}
          <button className="add-doc-button" onClick={() => openModal()}>{t('add')}</button> {/* Translated Add Document button */}
        </div>
